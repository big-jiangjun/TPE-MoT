...
def _get_perception_head(runner):
    model = runner.model
    if hasattr(model, "module"):
        model = model.module
    if hasattr(model, "perception_head"):
        return model.perception_head
...
class LoRAMergeHook(Hook):
    ...
    def after_run(self, runner):
        ...
        if not self._lora_enabled(runner):
            if rank == 0:
                runner.logger.info(
                    "[LoRAMergeHook] LoRA is not enabled on this run 鈥?skipping in-memory merge.")
            return
        ...
        if ph is None or not hasattr(ph, "merge_lora"):
            if rank == 0:
                runner.logger.warning(
                    "[LoRAMergeHook] Could not locate perception_head.merge_lora(). Skipping.")
            return
