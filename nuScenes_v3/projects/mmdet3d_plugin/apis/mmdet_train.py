...
def custom_train_detector(
    ...
    if deepspeed_enabled:
        ...
        if cfg.load_from:
            ...
            if isinstance(state_dict, dict) and isinstance(state_dict.get('module'), dict):
                state_dict = state_dict['module']
            if not isinstance(state_dict, dict):
                raise TypeError(
                    "DeepSpeed load_from must contain a state_dict or module dictionary; "
            ...
            logger.info(
                "[DeepSpeed] load_from completed: loaded=%d missing=%d unexpected=%d",
                len(new_state_dict),
    ...
    # LoRA Merge Hook: auto-register when lora_cfg is set in the perception head.
    ...
    _perception_head_cfg = cfg.get('model', {}).get('perception_head', {})
    if _perception_head_cfg.get('lora_cfg') is not None:
