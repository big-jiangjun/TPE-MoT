    ...
    def forward(
        ...
        **kwargs: dict,
        ...
        if self.use_deformable_func:
            ...
            # --- Code Change ---
            # Reason: DAF CUDA kernel reads/writes float* pointers. Under DeepSpeed bf16,
            # feature_maps are bf16 (2 bytes/elem) but kernel treats them as float32
            # (4 bytes/elem) → memory corruption. Fix: cast to float32 before DAF,
            # downstream .to() at L158 casts back automatically.
            features = DAF(feature_maps[0].float(), feature_maps[1], feature_maps[2],
                          points_2d.float(), weights.float()).reshape(
