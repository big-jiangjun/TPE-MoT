...
class LoadMultiViewImageFromFiles(object):
    ...
    # --- Code Change ---
    # Reason: 加 with_temporal 参数，多帧时逐帧加载嵌套 (T, 6) 图像列表
    def __init__(self, to_float32=False, color_type="unchanged", with_temporal=False):
    ...
    def __call__(self, results):
        ...
        # --- Code Change ---
        # Reason: with_temporal=True 且 img_filename[0] 是 list 时，表示 (T, 6) 嵌套结构，
        # 需逐帧逐相机加载，保持嵌套结构。单帧时走原逻辑。
        if self.with_temporal and isinstance(filename[0], list):
            # (T, 6) 嵌套: 逐帧加载
            imgs = []
            for frame_paths in filename:
                frame_imgs = [mmcv.imread(p, self.color_type) for p in frame_paths]
                if self.to_float32:
                    frame_imgs = [img.astype(np.float32) for img in frame_imgs]
                imgs.append(frame_imgs)
            results["img"] = imgs  # (T, 6) 嵌套 list
            # img_shape 取第一帧第一相机的形状作为参考
            first_img = imgs[0][0]
            results["img_shape"] = [first_img.shape] * len(imgs)
            results["ori_shape"] = first_img.shape
            results["pad_shape"] = first_img.shape
            results["scale_factor"] = 1.0
            num_channels = 1 if len(first_img.shape) < 3 else first_img.shape[2]
