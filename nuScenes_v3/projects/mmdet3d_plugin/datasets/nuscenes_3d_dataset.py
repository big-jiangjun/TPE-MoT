...
class NuScenes3DDataset(Dataset):
    ...
    def __init__(
        ...
        vad_ann_file=None,
        # --- Code Change ---
        # Reason: 支持时序视频输入——加载当前帧前的 N 帧历史帧
        ...
        # --- Code Change ---
        # Reason: 存储历史帧数并预建 scene_token 索引，加速 get_data_info 中按 scene 查找历史帧
        ...
        if self.num_history_frames > 0:
            ...
            for i, info in enumerate(self.data_infos):
                ...
                ts = info["timestamp"]
                tk = info["token"]
                self._scene_index.setdefault(st, []).append((i, ts, tk))
            # 每个 scene 内按 timestamp 降序排列（[newest, ..., oldest]），方便在 _get_history_frames 中
            # 从当前帧位置向后取历史帧
            for st in self._scene_index:
                self._scene_index[st].sort(key=lambda x: x[1], reverse=True)
        # --- End Code Change ---
    ...
    # --- Code Change ---
    # Reason: 在同 scene 内查找当前帧的历史帧（含 self），返回 current-last 顺序的帧索引列表。
    # 不足时 duplication padding（复制最旧帧），并生成 temporal_mask 标记 padding 帧。
    def _get_history_frames(self, info, num_history):
        """获取同 scene 内当前帧的历史帧索引列表（current-last 顺序）。

        返回:
            history_indices: [t-n, ..., t-1, cur] — 最旧→最新
            temporal_mask:   float list，历史帧=1.0, padding帧=0.0
        """
        scene_token = info["scene_token"]
        cur_ts = info["timestamp"]
        cur_token = info["token"]
        scene_frames = self._scene_index.get(scene_token, [])

        # 在降序列表中定位当前帧（按 token 精确匹配）
        cur_pos = None
        for p, (idx, ts, tk) in enumerate(scene_frames):
            if tk == cur_token:
                cur_pos = p
                break
        if cur_pos is None:
            # fallback: 按 timestamp 比较（场景切换边界等极端情况）
            for p, (idx, ts, tk) in enumerate(scene_frames):
                if ts <= cur_ts:
                    cur_pos = p
                    break
        ...
        # 降序列表 [cur, t-1, t-2, ...] → 取 cur_pos 向后 num_history+1 帧
        # → reverse 为 current-last: [t-n, ..., t-1, cur]
        candidates = scene_frames[cur_pos:cur_pos + num_history + 1]
        history_indices = [idx for idx, ts, tk in reversed(candidates)]

        # Duplication Padding: 不足时复制最旧帧（history_indices[0]）
        while len(history_indices) < num_history + 1:
            history_indices.insert(0, history_indices[0])

        # temporal_mask: padding 帧为 0（padding 帧在最前面）
        temporal_mask = [1.0] * len(history_indices)
        num_padding = len(history_indices) - len(candidates)
        for i in range(num_padding):
            temporal_mask[i] = 0.0

        return history_indices, temporal_mask
    # --- End Code Change ---
    ...
    def get_data_info(self, index):
        ...
        if self.modality["use_camera"]:
            # --- Code Change ---
            # Reason: 多帧模式下构建嵌套 (T, 6) 结构（current-last 顺序: [t-3, t-2, t-1, cur]）。
            # 逐帧手动计算 lidar2global（从 lidar2ego/ego2global 分量构造，因 data_infos 不含此字段），
            # 并存为 hist_T_global_inv 供 Phase 4 路径 A ego 位姿补偿使用。
            if self.num_history_frames > 0:
                frame_indices, temporal_mask = self._get_history_frames(info, self.num_history_frames)
                image_paths = []
                lidar2img_rts = []
                lidar2cam_rts = []
                cam_intrinsic = []
                timestamps = []
                hist_T_global_inv = []  # 路径 A ego 补偿需要：逐帧 lidar2global 的逆矩阵

                for idx_in_seq, fi in enumerate(frame_indices):
                    fi_info = self.data_infos[fi]
                    timestamps.append(fi_info["timestamp"] / 1e6)

                    # 计算逐帧 lidar2global（与单帧 L406-416 逻辑一致）
                    _l2e = np.eye(4)
                    _l2e[:3, :3] = pyquaternion.Quaternion(fi_info["lidar2ego_rotation"]).rotation_matrix
                    _l2e[:3, 3] = np.array(fi_info["lidar2ego_translation"])
                    _e2g = np.eye(4)
                    _e2g[:3, :3] = pyquaternion.Quaternion(fi_info["ego2global_rotation"]).rotation_matrix
                    _e2g[:3, 3] = np.array(fi_info["ego2global_translation"])
                    _l2g = _e2g @ _l2e
                    hist_T_global_inv.append(np.linalg.inv(_l2g))

                    fi_paths = []
                    fi_l2i = []
                    fi_l2c = []
                    fi_ci = []
                    for cam_type, cam_info in fi_info["cams"].items():
                        fi_paths.append(cam_info["data_path"])
                        ...
                        fi_ci.append(intrinsic)
                        viewpad = np.eye(4)
                        viewpad[: intrinsic.shape[0], : intrinsic.shape[1]] = intrinsic
                        lidar2img_rt = viewpad @ lidar2cam_rt.T
                        fi_l2i.append(lidar2img_rt)
                        fi_l2c.append(lidar2cam_rt)
                    image_paths.append(fi_paths)
                    lidar2img_rts.append(fi_l2i)
                    lidar2cam_rts.append(fi_l2c)
                    cam_intrinsic.append(fi_ci)

                input_dict.update(
                    dict(
                        img_filename=image_paths,           # (T, 6) 嵌套 list
                        lidar2img=lidar2img_rts,            # (T, 6, 4, 4)
                        lidar2cam=lidar2cam_rts,            # (T, 6, 4, 4)
                        cam_intrinsic=cam_intrinsic,        # (T, 6, 3, 3)
                        timestamp=timestamps,               # [T] float list
                        hist_T_global_inv=hist_T_global_inv,  # [T] 每帧 lidar2global 逆, 路径 A ego 补偿用
                        temporal_mask=temporal_mask,        # [T] float list (padding=0.0, real=1.0)
                    )
                )
            else:
            # --- End Code Change ---
    ...
    def get_ann_info(self, index):
        ...
        if "instance_inds" in info:
            # --- Code Change ---
            # np.int 在 numpy 2.x 中已移除；使用内置 int
            # instance_inds = np.array(info["instance_inds"], dtype=np.int)[mask]
            instance_inds = np.array(info["instance_inds"], dtype=int)[mask]
            # --- End Code Change ---
