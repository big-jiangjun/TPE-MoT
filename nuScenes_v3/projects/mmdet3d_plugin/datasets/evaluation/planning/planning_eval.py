...
class PlanningMetricLoose():
    ...
    TPE-MoT/infer_nusc_vlm/eval/evaluation.py + metric.py.
    ...
    def evaluate_single_coll(self, traj, segmentation):
        ...
        trajs[:, :, [0, 1]] = trajs[:, :, [1, 0]]          # swap x鈫攜 for BEV
...
# ---------------------------------------------------------------------------
# Print helpers 鈥?two display conventions
# ---------------------------------------------------------------------------
...
def _get_occupancy(occ_map, token):
    ...
    if occ is None:
        return None
    if occ.ndim == 4:
        occ = occ.squeeze(0)          # (1,T,200,200) 鈫?(T,200,200)
...
def planning_eval(results, eval_config, logger, seg_pkl_path=None):
    ...
    _print_strict_format(
        ...
        'Planning Metrics  [SparseDrive 鈥?STRICT]  '
    ...
    if m_loose is not None:
        ...
        _print_uniad_format(
            ...
            'Planning Metrics  [GPT-Driver 鈥?UniAD]  '
        ...
        metric_dict = _print_stp3_format(
            loose_results, logger,
            'Planning Metrics  [GPT-Driver 鈥?STP-3]  '
