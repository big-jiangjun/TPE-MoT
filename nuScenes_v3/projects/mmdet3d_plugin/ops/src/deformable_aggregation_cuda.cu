...
__global__ void deformable_aggregation_kernel(
    ...
    // --- Code Change ---
    // Reason: int64_t prevents overflow when num_kernels > INT_MAX (2.147B).
    // With bs=8, num_cams=12, MAP num_pts=300 → 2.95B threads overflows int32.
...
__global__ void deformable_aggregation_grad_kernel(
    ...
    // --- Code Change ---
    // Reason: int64_t prevents overflow when num_kernels > INT_MAX. See forward kernel.
    int64_t idx = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
    // --- End Code Change ---
    if (idx >= num_kernels) return;
...
void deformable_aggregation(
    ...
    // --- Code Change ---
    // Reason: (int64_t) cast prevents signed int32 overflow in the multiplication
    // chain. With bs=8, num_cams=12, MAP num_pts=300 → 2.95B > INT_MAX (2.147B).
    const int64_t num_kernels = (int64_t)batch_size * num_pts * num_embeds * num_anchors * num_cams * num_scale;
    // Clear any latent CUDA error before kernel launch
    cudaGetLastError();
    ...
    if (err != cudaSuccess) {
        std::cerr << "CUDA error after deformable_aggregation_kernel: "
...
void deformable_aggregation_grad(
  ...
  int num_groups
    ...
    // --- Code Change ---
    // Reason: (int64_t) cast prevents signed int32 overflow. See forward host function.
    const int64_t num_kernels = (int64_t)batch_size * num_pts * num_embeds * num_anchors * num_cams * num_scale;
    cudaGetLastError();
    ...
    cudaError_t err = cudaDeviceSynchronize();
    if (err != cudaSuccess) {
        std::cerr << "CUDA error after deformable_aggregation_grad_kernel: "
                  << cudaGetErrorString(err) << std::endl;
    }
    // --- End Code Change ---
}
