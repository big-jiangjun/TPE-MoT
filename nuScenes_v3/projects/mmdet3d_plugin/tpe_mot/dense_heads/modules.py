...
class OccLatentDecoder(nn.Module):
    ...
    def forward(self, occ_tokens):
        ...
        target_shape = (B, 1, 200, 200, 16)  # target_shape = (B, 1, 200, 200, 16) 的含义是：B 是批量大小，1 是通道数（单通道），200x200 是空间分辨率，16 是高度维度（z_channels）。这个形状表示每个批次的输出是一个 200x200 的网格，每个网格单元包含一个 16 维的潜在表示。
